...
3. Input cmd "python -m venv .venv" to create a virtual environment
    a. Name of ".venv" is not required, but used as it is the default venv name in VSCode
4. Enter the cmd ".venv/Scripts/activate" to activate the virtual environment
    a. If using VSCode, the name of your virtual environment should appear in green before the working tree in the terminal
5. Enter "pip install -r requirements.txt" to install required libraries
6. To run program, enter "python main.py" into cmd
    a. The game should be opened into the main page of a career (where trainers can select actions to take)